...
export function applyFx(state, fx) {
  ...
  if (fx.day) {
    s.day += fx.day;
    if (s.buildings.find((b) => b.id === 'small_farm' && b.built))
      s.food = Math.max(0, s.food + fx.day * 5);
    if (s.buildings.find((b) => b.id === 'woodcutter' && b.built))
      s.wood = Math.max(0, s.wood + fx.day * 15);
    if (s.buildings.find((b) => b.id === 'quarry' && b.built))
      s.stone = Math.max(0, s.stone + fx.day * 12);
  ...
  if (fx.npcs)
    fx.npcs.forEach((ch) => {
      ...
      if (i >= 0) {
        if (ch.l !== undefined)
          s.npcs[i].l = Math.max(0, Math.min(100, s.npcs[i].l + (ch.l || 0)));
        if (ch.t !== undefined)
          s.npcs[i].t = Math.max(0, Math.min(100, s.npcs[i].t + (ch.t || 0)));
        if (ch.a !== undefined && s.npcs[i].a !== null)
          s.npcs[i].a = Math.max(0, Math.min(100, (s.npcs[i].a || 0) + (ch.a || 0)));
      }
    });
